...
from apps.tramite.tablas.personaldespacho import TablaPersonalDespacho
from apps.tramite.vistas.bandejas import BandejaVista, BandejaListarFeedDataView


class PersonalBandejaDespacho(BandejaVista):
    template_name = "tramite/personal/despacho/vista.html"

    def post(self, request, *args, **kwargs):
        context = self.get_context_data(**kwargs)
        context["TablaPersonalDespacho"] = TablaPersonalDespacho(request=request)
        return self.render_to_response(context=context)


class PersonalBandejaDespachoListar(BandejaListarFeedDataView):
    table = "tablas.personaldespacho.TablaPersonalDespacho"
    qs = "QueryPersonalBandejaDespacho"


class PersonalBandejaDespachoFirmaMasiva(TemplateValidaLogin, TemplateView):
    template_name = "tramite/personal/despacho/firmamasiva.html"

    def get_context_data(self, **kwargs):
        context = super(PersonalBandejaDespachoFirmaMasiva, self).get_context_data(**kwargs)
        ids = eval(self.request.GET.get("ids").replace("_", ","))
        context["documentos"] = Documento.objects.filter(pk__in=ids)
        return context


class PersonalBandejaDespachoFirmaMasivaEjecutar(TemplateValidaLogin, TemplateView):
    template_name = "tramite/personal/despacho/firmamasivaejecutar.html"
    ...
    def render_to_response(self, context, **response_kwargs):
        codigos = eval(self.request.POST.get("codigos").replace("_", ","))
            ...
            if doc.FirmaTitular() and doc.ultimoestado.estado not in ["OF"]:
                ...
                if not pdfJson:
                    ...
                    pdfJson = {
                        ...
                        "rutabajada": "%s%s" % (
                            dochost,
                            reverse("apptra:personal_bandeja_despacho_firmamasiva_bajar", kwargs={"codigo": codigouuid})
                        ),
                        "rutasubida": "%s%s" % (
                            dochost,
                            reverse("apptra:personal_bandeja_despacho_firmamasiva_subir", kwargs={
                                "codigo": codigouuid,
                                "emitir": int(emitir)
                            })
                        ),
                        "rutaerror": "%s%s" % (
                            dochost,
                            reverse("apptra:personal_bandeja_despacho_firmamasiva_error", kwargs={"codigo": codigouuid})
                        ),
                        "rutalogos": dochost,
                        "logos": listalogos(),
        ...
        return super(PersonalBandejaDespachoFirmaMasivaEjecutar, self).render_to_response(context, **response_kwargs)


class PersonalBandejaDespachoFirmaMasivaBajar(views.APIView):
...
class PersonalBandejaDespachoFirmaMasivaSubir(views.APIView):
    authentication_classes = [TokenAuthentication]
    permission_classes = [IsAuthenticated]
    http_method_names = "post"
...
class PersonalBandejaDespachoFirmaMasivaError(views.APIView):
    authentication_classes = [TokenAuthentication]
    permission_classes = [IsAuthenticated]
    http_method_names = "post"
...
class PersonalBandejaDespachoEmisionMasiva(TemplateValidaLogin, TemplateView):
    template_name = "tramite/personal/despacho/emisionmasiva.html"
    extra_context = {
        "botonguardartexto": "<i class='fas fa-paper-plane fa-1x'></i> Emitir"
    }

    def get_context_data(self, **kwargs):
        context = super(PersonalBandejaDespachoEmisionMasiva, self).get_context_data(**kwargs)
        if self.request.GET:
            ids = eval(self.request.GET.get("ids").replace("_", ","))
            context["ids"] = ids
            context["documentos"] = Documento.objects.filter(pk__in=ids)
