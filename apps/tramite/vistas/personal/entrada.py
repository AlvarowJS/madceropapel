...
from apps.tramite.tablas.personalentrada import TablaPersonalEntrada
from apps.tramite.vistas.bandejas import BandejaVista, BandejaListarFeedDataView


class PersonalBandejaEntrada(BandejaVista):
    template_name = "tramite/personal/entrada/vista.html"

    def post(self, request, *args, **kwargs):
        context = self.get_context_data(**kwargs)
        context["TablaPersonalEntrada"] = TablaPersonalEntrada(request=request)
        return self.render_to_response(context=context)


class PersonalBandejaEntradaListar(BandejaListarFeedDataView):
    table = "tablas.personalentrada.TablaPersonalEntrada"
    qs = "QueryPersonalBandejaEntrada"


class PersonalBandejaEntradaRecepcionMasiva(TemplateValidaLogin, TemplateView):
    template_name = "tramite/personal/entrada/recepcionmasiva.html"
    extra_context = {
        "botonguardartexto": "<i class='fas fa-paper-plane fa-1x'></i> Recepcionar"
    }

    def get_context_data(self, **kwargs):
        context = super(PersonalBandejaEntradaRecepcionMasiva, self).get_context_data(**kwargs)
