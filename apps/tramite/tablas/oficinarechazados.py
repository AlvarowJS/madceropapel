...
class ColumnaAccionRechazo(Columna):
    def __init__(self):
        super(ColumnaAccionRechazo, self).__init__(
            field="pk", header="", sortable=False, searchable=False, attrs={"class": "w-80px"}, position="center"
        )
    ...
    def render(self, obj):
        ...
        kwargs = {"pk": obj.pk}
        # Volver a Enviar
        _urlreenviar = reverse("apptra:oficina_bandeja_rechazado_reenviar", kwargs=kwargs)
        _result += _boton % (
            "primary",
            _urlreenviar,
            'Enviar Nuevamente',
        ...
        if obj.documento.des_documento.exclude(pk=obj.pk).count() > 0:
            _urlranular = reverse("apptra:oficina_bandeja_rechazado_anular", kwargs=kwargs)
            _result += _boton % (
                "danger",
                _urlranular,
        ...
        # Archivar
        _urlarchivar = reverse("apptra:oficina_bandeja_rechazado_archivar", kwargs=kwargs)
        _result += _boton % (
            "info",
            _urlarchivar,
            'Archivar',
...
class TablaOficinaRechazados(Table):
    ...
    def __init__(self, request, *args, **kwargs):
        super(TablaOficinaRechazados, self).__init__(request, *args, **kwargs)
        self.opts.ajax_source = reverse(viewname="apptra:oficina_bandeja_rechazado_listar")
